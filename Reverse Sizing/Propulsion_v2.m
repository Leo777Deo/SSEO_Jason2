...
% DV requested
total_DV=130*2; % 130 m/s with 100% margin (stochastic maneuvers)

List_ISP=[0,220,0]; % [s] list of specific impulse per propellant type 
...
List_P_gas_i=linspace(5.5*1e5,25.4*1e5,nbr_points); 
List_P_gas_f=zeros(nbr_points,1);
List_m_press_He=zeros(nbr_points,1);
List_m_press_N2=zeros(nbr_points,1);
List_r_tank=zeros(nbr_points,1);
List_t_tank=zeros(nbr_points,1);
List_m_tank=zeros(nbr_points,1);
List_dP_feed=zeros(nbr_points,1);

% Defining the pressure system characteristics
dP_feed=50*1e3; % [Pa] 
P_inlet=5.5*1e5; %[Pa] feed pressure of the thruster
...
sigma=880*1e6; % strength Ti6Al4V
rho_tank=4420; % density Ti6Al4V

for i = 1:nbr_points
    
    P_tank_i=List_P_gas_i(i);
    P_tank_f=P_tank_i/B;
    if P_tank_f>5.5*1e5
        ...
        t_tank=P_burst*r_tank/(2*sigma);% PROBLEM WITH THE thickness
...
n=min(find(List_dP_feed>50*1e3));
...
v_pipe=sqrt(List_dP_feed(n)*2/rho_prop);% v_pipe=m_pipe/(A*rho_prop);
...
if Re>2300
    f=colebrook(Re) % friction factor for turbulent flow
else 
    f=64/Re
end
fprintf('Length of the pipe [m]: \n')
L=List_dP_feed(n)*2*d/(f*rho_prop*v_pipe^2)
...
subplot(2,3,1);
hold on
scatter(List_P_gas_i(:),List_P_gas_f(:),'DisplayName',"Final pressure in tank")
plot(List_P_gas_i(n),List_P_gas_f(n),"LineWidth",2,"Marker","+",'HandleVisibility','off')
set(gca,'fontsize',12)
ylabel('Pressure [bar]','Interpreter','latex')
xlabel('Initial pressure in tank [bar]','Interpreter','latex')
grid on
legend show
%
...
% %Volume of tank
% subplot(2,3,2);
% plot(List_P_gas_i(:),List_V_tank*1e3,'DisplayName',"Volume of tank")
% set(gca,'fontsize',12)
% ylabel('Volume [L]','Interpreter','latex')
% xlabel('Initial pressure in tank [bar]','Interpreter','latex')
% grid on
% legend show
% 
% %  Diameter of tank
% subplot(2,3,3);
% plot(List_P_gas_i(:),List_r_tank*2*1e3,'DisplayName',"Diameter of tank")
% set(gca,'fontsize',12)
% ylabel('Diameter [mm]','Interpreter','latex')
% xlabel('Blowdown ratio [-]','Interpreter','latex')
% grid on
% legend show
...
subplot(1,3,2);
hold on
plot(List_P_gas_i(:),sqrt(List_dP_feed(:)*2/rho_prop),'DisplayName',"Flow velocity in pipe")
plot(List_P_gas_i(n),sqrt(List_dP_feed(n)*2/rho_prop),"LineWidth",2,"Marker","+",'HandleVisibility','off')
set(gca,'fontsize',12)
...
subplot(1,3,3);
hold on
plot(List_P_gas_i(:),List_dP_feed(:)*1e-5,'DisplayName',"Dynamic pressure in pipe")
plot(List_P_gas_i(n),List_dP_feed(n)*1e-5,"LineWidth",2,"Marker","+",'HandleVisibility','off')
set(gca,'fontsize',12)
ylabel('Pressure [bar]','Interpreter','latex')
xlabel('Initial pressure in tank [bar]','Interpreter','latex')
grid on
